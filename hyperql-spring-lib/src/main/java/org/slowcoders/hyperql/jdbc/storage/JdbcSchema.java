...
public class JdbcSchema extends QSchema {
    ...
    public static void dumpJPAHeader(SourceWriter sb, boolean includeJsonType, boolean isJPA) {
        if (includeJsonType) {
        ...
        if (isJPA) {
            ...
            sb.writeln("import org.slowcoders.hyperquery.core.*;");
        }
        sb.writeln();
        sb.writeln("/** This source is generated by HQL-JDBC */\n");
    }

    public void dumpJPAEntitySchema(SourceWriter sb, boolean includeHeader, boolean isJPA) {
        if (includeHeader) {
            dumpJPAHeader(sb, !this.getExtendedColumns().isEmpty(), isJPA);
        }
        ...
        if (isJPA) {
            ...
            if (isMultiPKs) {
                ...
                for (QColumn column : getPKColumns()) {
                    dumpColumnDefinition(column, sb, isJPA);
                }
                sb.decTab();
                sb.decTab();
            }
        } else {
            sb.writeln("public class " + className + " implements QEntity<" + className + "> {");
        }
        sb.incTab();
        //idColumns = getIDColumns();
        for (QColumn col : getReadableColumns()) {
            dumpColumnDefinition(col, sb, isJPA);
            sb.writeln();
        }

        for (Map.Entry<String, QJoin> entry : this.getEntityJoinMap().entrySet()) {
            QJoin join = entry.getValue();
            dumpJoinedColumn(join, sb, isJPA);
    ...
    private void dumpColumnDefinition(QColumn col, SourceWriter sb, boolean isJPA) {
        if (isJPA && col.getJoinedPrimaryColumn() != null) return;
        ...
        String fieldName = isJPA ? getJavaFieldName(col)
                        : CaseConverter.toCamelCase(col.getPhysicalName(), false);
...
//        }
    ...
    private void dumpJoinedColumn(QJoin join, SourceWriter sb, boolean isJPA) {
        boolean isInverseJoin = join.isInverseMapped();
        QColumn firstFk = join.getJoinConstraint().get(0);
        if (isJPA && isInverseJoin && join.getAssociativeJoin() == null && firstFk.getSchema().hasOnlyForeignKeys()) {
            return;
        }
        ...
        if (isJPA) {
            ...
            sb.write("public static QJoin ").write(getJavaFieldName(join));
            sb.write(" = QJoin.of(").write(mappedType).write(".class, ");
            JoinConstraint srcColumns = join.getJoinConstraint();
            for (QColumn col : srcColumns) {
                QColumn tmp = col.getJoinedPrimaryColumn();
                QColumn pk;
                if (isInverseJoin) {
                    pk = tmp;
                } else {
                ...
                if (pk.getPhysicalName().equals(col.getPhysicalName())) {
                    sb.writeQuoted(col.getPhysicalName()).write(", ");
                } else {
                    sb.write("\"#.").write(col.getPhysicalName()).write(" = ");
                    sb.write("@.").write(pk.getPhysicalName()).write("\",\n");
                }
            }
            sb.shrinkLength(2);
            sb.write(");\n\n");
    ...
    private void dumpTableDefinition(SourceWriter sb, boolean isJPA) {
        if (isJPA) {
            ...
            sb.write("@QFrom(").writeQuoted(this.getTableName()).write(")\n");
        }
    }
